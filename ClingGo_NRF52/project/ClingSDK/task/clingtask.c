...
#define CORE_FSM_TASK_PRIORITY                        ( OS_TASK_PRIORITY_NORMAL  )
...
void cling_init_task(void *pvParameters)
{
        ...
        OS_TASK_CREATE("BLE Peripheral", /* The text name assigned to the task, for debug only; not used by the kernel. */
...
static void cling_task_core_fsm(void *pvParameters)
{
        ...
        while (1) {
                ...
                N_SPRINTF("CLING TASK: event: 0x%08x @ %d", notified_value, CLK_get_system_time());
