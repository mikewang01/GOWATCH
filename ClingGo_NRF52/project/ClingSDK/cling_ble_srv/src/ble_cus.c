...
ble_service_t *cling_service_init(const ble_service_config_t *cfg, TASK_HANDLE_SETS *sets)
{
        ...
        int ret   __attribute__((unused)) = ble_gatts_add_service(&uuid, GATT_SERVICE_PRIMARY, num_attr);

        Y_SPRINTF("[ble cust]: ble_gatts_add_service ret = 0x%04x\r\n", ret);
        fflush(stdout);
...
{
        OS_TASK_NOTIFY(task_handles->cling_core_fsm_handle, 0x02, OS_NOTIFY_SET_BITS);
}
