 ...
 */
static const spi_device_config *current_config[2];
...
void ad_spi_device_init(const spi_device_id id)
{
        ...
        config->data->dev_res_id = (RES_ID)resource_add();
#endif
}
...
void ad_spi_complex_transact(spi_device dev, spi_transfer_data *transfers, size_t count)
{
        ...
        hw_spi_writeread_buf(id, transfers[i].wbuf, transfers[i].length, transfers[i+1].rbuf,
                                                transfers[i+1].length, ad_spi_wait_event, device);
        OS_EVENT_WAIT(device->bus_data->event, OS_EVENT_FOREVER, ret);
...
void ad_spi_async_transact(spi_device dev, ...)
{
         ...
         */
        do {
                action = va_arg(ap, uint32_t);
                data->transaction[i++] = action;
                switch (action & 0xFF000000) {
                case SPI_TAG_SEND_RECEIVE:
                        data->transaction[i++] = va_arg(ap, uint32_t);
                        /* no break */
                case SPI_TAG_CALLBACK0:
                case SPI_TAG_SEND:
                case SPI_TAG_RECEIVE:
...
static bool ad_spi_prepare_for_sleep(void)
{
        ...
        if (current_config[0]) {
                current_config[0]->bus_data->current_device = NULL;
								hw_spi_disable(current_config[0]->bus_id);
        }
        if (current_config[1]) {
                current_config[1]->bus_data->current_device = NULL;
								hw_spi_disable(current_config[1]->bus_id);
        }
