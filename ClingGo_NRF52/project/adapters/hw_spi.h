 ...
 *
 */
#define HW_SPI1         ((void *)NRF_SPI0)
#define HW_SPI2         ((void *)NRF_SPI1)
 ...
 */
typedef enum {
        HW_SPI_PHA_MODE_0 = NRF_DRV_SPI_MODE_0,
        HW_SPI_PHA_MODE_1 = NRF_DRV_SPI_MODE_1,
				HW_SPI_PHA_MODE_2 = NRF_DRV_SPI_MODE_2,
				HW_SPI_PHA_MODE_3 = NRF_DRV_SPI_MODE_3,
} HW_SPI_PHA;
 ...
 */
typedef enum {
        HW_SPI_FREQ_2M = NRF_SPI_FREQ_2M,      /**< (XTAL or PLL/2)/(PER_DIV20 *8) */
        HW_SPI_FREQ_4M = NRF_SPI_FREQ_4M,      /**< (XTAL or PLL/2)/(PER_DIV20 *4) */
        HW_SPI_FREQ_8M = NRF_DRV_SPI_FREQ_8M,      /**< (XTAL or PLL/2)/(PER_DIV20 *2) */
        HW_SPI_FREQ_1M = NRF_SPI_FREQ_1M,     /**< (XTAL or PLL/2)/(PER_DIV20 *14) */
} HW_SPI_FREQ;
 ...
 *
 */
#define HW_GPIO_PORT uint32_t
#define HW_GPIO_PIN	uint32_t
typedef struct
{
        HW_GPIO_PORT port;
        HW_GPIO_PIN pin;
...
static inline uint16_t hw_spi_fifo_read16(HW_SPI_ID id)
{
        ...
        return 1;//SBA(id)->SPI_RX_TX_REG0;
}
...
static inline uint8_t hw_spi_fifo_read8(HW_SPI_ID id)
{
        ...
        return 1;//(uint8_t) SBA(id)->SPI_RX_TX_REG0;
}
...
static inline uint32_t hw_spi_fifo_read32(HW_SPI_ID id)
{
        ...
        // Read byte from the SPI RX register or RX FIFO
        return 1;//SBA(id)->SPI_RX_TX_REG0 | res;
}
...
{
        return 1;//HW_SPI_REG_GETF(id, SPI_CTRL_REG1, SPI_PRIORITY);
}
...
static inline uint8_t hw_spi_is_busy(HW_SPI_ID id)
{
        ...
        return 1;//HW_SPI_REG_GETF(id, SPI_CTRL_REG1, SPI_BUSY);
}
